...
- **`escapeHtml` helper in server scripts** — every `data-bascik-server` script now has `escapeHtml(value)` available automatically (no import needed). It HTML-escapes `&`, `<`, `>`, and `"` so user-controlled values from headers, cookies, query params, or database rows can be safely interpolated into HTML without adding a custom escape helper.
