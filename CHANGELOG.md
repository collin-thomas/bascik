...
- **Netlify build OOM** — sequential (non-concurrent) page processing and build-script execution prevent the ~50+ simultaneous Node.js child processes that previously exhausted the 2 GB Netlify build container. Pages and their build scripts now run one at a time.
