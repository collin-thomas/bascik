...
- **`bascikEsc` helper in server scripts** — every `data-bascik-server` script now has `bascikEsc(value)` available automatically (no import needed). It HTML-escapes `&`, `<`, `>`, and `"`, eliminating the need to define an inline escape helper for user-controlled values from headers, cookies, query params, or database rows.
