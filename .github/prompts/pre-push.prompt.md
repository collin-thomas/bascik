...
4. Run typechecks, checks, unit tests, Playwright E2E tests, and Lighthouse CLI audits across all monorepo packages using token-efficient reporters:
   ...
   - Parse test results efficiently: check the pass/fail status first. If all tests pass, do NOT read or summarize individual passing test lines. If any test or audit fails, inspect ONLY the failure stack traces, Lighthouse score violations, or error messages, and fix any performance, accessibility, best practices, or SEO issues found before pushing.
5. `yarn coverage:all`
