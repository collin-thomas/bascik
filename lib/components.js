...
// Find the ids of getElementById /(?<=getElementById\([\'|\"]).+?(?=[\'|\"]\))/gim
// Find the ids of a querySelector /(?<=querySelector\([\'|\"]\[id=[\'|\"]).+?(?=[\'|\"]\][\'|\"]\))/gim
...
export const scopeTag = ({ component }) => {
  ...
  // unfinished
  if (ids === null || !Array.isArray(ids)) return fileContent;
 ...
 * @returns
 */
export const generateRandomHash = (length = 9) => {
