...
const prefixClassesInCss = (css, componentName) => {
  return css.replace(
    /(?<=\.)[a-z0-9-_]+/gim,
    (className) => `bascik__${componentName}__${className}`
  );
};
...
const convertCssElementSelectorsToClasses = (css, componentName) => {
  ...
  const cssStr = css.replace(/^[a-z1-6]+(?=[\s{])/gim, (elementName) => {
    ...
    return `.bascik__${componentName}__el__${elementName}`;
  });
      ...
      (elementHtml) => {
        ...
        if (elementHtml.match('class="')) {
          elementHtml = elementHtml.replace(
            /class=".*?(?=")/i,
            (classStr) => `${classStr} bascik__${componentName}__el__${element}`
          );
        } else {
          // Otherwise set the element class as the only class
          elementHtml = elementHtml.replace(
            new RegExp(`<${element}`, "i"),
            `<${element} class="bascik__${componentName}__el__${element}"`
          );
        }
        return elementHtml;
      }
...
const removeCommentsFromCss = (css) => {
  return css.replace(/\/\*[\s\S]*?\*\//gim, "");
};
