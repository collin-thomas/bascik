...
import {  prefixKeyframes, removeIdSelectors } from "./styles.js";
...
export const prefixElementAttribute = (component, attribute) => {
  ...
  if (attribute === 'class' && component.cssFileContent) {
    ...
    component.cssFileContent = component.cssFileContent.replace(/(?<=\.)[a-z0-9-_]+/gim, (className) => {
     return obfuscateAttributeName(`bascik__${component.name}__${componentInstanceId}__${className}`)
    })

    component.cssFileContent = prefixKeyframes(component.cssFileContent, `${component.name}__${componentInstanceId}`)
