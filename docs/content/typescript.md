...
Bascik is written in TypeScript, and projects built with Bascik can be written in TypeScript too. Add `data-bascik-ts` to any inline `<script>` tag — client, build-time, or server — and Bascik handles the rest. No bundler, no compile step, no configuration.
