...
| `:nth-child(An+B of .selector)`                    | `:nth-child(2n+1 of .item) {}`          | –     | Class names in the `of <selector>` filter argument are not scoped.                                                                                                                                                                                                                        |
...
| `@property`               | –     | Registered custom property names (e.g. `@property --brand {}`) are not scoped. The `--brand` identifier in the `@property` block and any `var(--brand)` references in the same component will not be renamed. Declare `@property` in a shared global stylesheet for now.                |
| `@starting-style`         | –     | Class names and element selectors inside `@starting-style` blocks are not scoped. Wrap rules in a class anchor (`.foo { @starting-style { … } }`) as a workaround.                                                                                                                       |
| `@counter-style`          | –     | Custom counter names declared with `@counter-style` are not scoped. References in `list-style`, `counter()`, and `counters()` will not match a scoped name.                                                                                                                               |
| `view-transition-name`    | –     | The value of the `view-transition-name` property is not scoped. Names may collide between multiple components on the same page. Use globally unique names manually.                                                                                                                       |
| `anchor-name` / `@position-try` | –  | Anchor names declared with `anchor-name` and referenced via `position-anchor` or `@position-try` block identifiers are not scoped. Use globally unique names manually.                                                                                                                   |
