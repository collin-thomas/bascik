...
export const getFilePosition = (filePath, searchString, tagName) => {
    try {
 ...
 */
const applyTransforms = (component, transforms) => transforms.reduce((c, fn) => fn(c), component);
/**
 ...
 */
const buildScopingPipeline = (instanceId) => {
    const skip = BascikConfig.skipTranspilingElementContents;
    return [
        BascikConfig.scopeAttribute.id &&
            ((c) => prefixElementAttribute(c, "id", instanceId, true, skip)),
        BascikConfig.scopeAttribute.name &&
            ((c) => prefixElementAttribute(c, "name", instanceId, true, skip)),
        BascikConfig.scopeAttribute.class &&
            ((c) => prefixElementAttribute(c, "class", instanceId, BascikConfig.deduplicateCss, skip)),
...
export const findActiveSourceFile = (html, index, fallback) => {
...
export const recursivelyTranspile = (transpiledHtmlBody, componentList, usedComponents = [], filePath) => {
    ...
    try {
        ...
        let transpiledTag = component.fileContent.replace(/<(\w+(?:-\w+)*)\s+data-bascik-slot(?!\s*=)((?:\s[^>]*)?)>([\s\S]*?)<\/\1>/gi, (_match, _tag, _extraAttrs, innerFallback) => defaultSlotContent || innerFallback);
        ...
        transpiledHtmlBody = replaceTag(transpiledHtmlBody, component.name, transpiledTag);
        usedComponents.push(component);
    }
    catch (error) {
        const activeSourceFile = findActiveSourceFile(transpiledHtmlBody, component.index || 0, filePath || "");
    ...
    return recursivelyTranspile(transpiledHtmlBody, componentList, usedComponents, filePath);
};
...
export const processAllPages = async () => {
    ...
    const results = await Promise.all(pageList.map((path) => {
...
export const pageProcessing = async (pagePath, componentList) => {
    ...
    if (!body) {
        console.warn(`warning: ${pagePath} does not contain <body></body> or body does not have content`);
        return;
    }
    let { transpiledHtmlBody, usedComponents } = recursivelyTranspile(body, componentList, [], pagePath);
    ...
    let { transpiledHtmlBody: transpiledHeadContent, usedComponents: headUsedComponents, } = recursivelyTranspile(headRaw ?? "", componentList, [], pagePath);
    ...
    // matching component file and will appear unresolved in the output HTML.
    {
        ...
        if (unresolved.size > 0) {
            console.warn(`[bascik] Unresolved component tag${unresolved.size > 1 ? "s" : ""} in "${relativePagePath}": ` +
