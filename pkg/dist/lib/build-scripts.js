...
const runModule = (path) => new Promise((resolve, reject) => {
    execFile(process.execPath, [path], { cwd: process.cwd() }, (err, stdout, stderr) => {
        if (err)
...
const BUILD_SCRIPT_RE = /<script\b[^>]*\sdata-bascik-build\b[^>]*>([\s\S]*?)<\/script>/gi;
/**
 ...
 */
export const executeBuildScripts = async (html, filePath) => {
    ...
    for (const match of matches) {
        ...
        const tmpPath = join(tempDir, `build-${Date.now()}-${Math.random().toString(36).slice(2)}.mjs`);
        try {
