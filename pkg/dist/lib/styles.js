...
export const convertCssElementSelectorsToClasses = (css, componentName) => {
    ...
    const toClass = (elementName) => {
        ...
        if (CSS_UNIT_KEYWORDS.has(elementName.toLowerCase()))
            return elementName;
    ...
    result = result.replace(/(?<=&\s+(?:[>+~]\s+)?)[a-z1-6]+(?=[^{};)]*\{)/g, toClass);
    return { css: result, elementsConvertedClasses };
};
/**
 * If a component's css styles any element, add bascik classes to those elements
 */
export const addElementClassesInHtml = (componentHtml, componentName, elementsConvertedClasses = []) => {
    ...
    elementsConvertedClasses.forEach((element) => {
        ...
        componentHtml = componentHtml.replace(new RegExp(`<${element}[^>]*>([\\s\\S]*?)<\\/${element}>`, "gis"), (elementHtml) => {
            ...
            if (elementHtml.match('class="')) {
                elementHtml = elementHtml.replace(/class=".*?(?=")/i, (classStr) => {
                    const bascikClassName = obfuscateAttributeName(`bascik__${componentName}__el__${element}`);
                ...
                const bascikClassName = obfuscateAttributeName(`bascik__${componentName}__el__${element}`);
                elementHtml = elementHtml.replace(new RegExp(`<${element}`, "i"), `<${element} class="${bascikClassName}"`);
            }
            return elementHtml;
        });
    });
...
export const prefixKeyframes = (css, componentName) => {
    ...
    if (!Array.isArray(keyframeNames))
        return css;
    return css.replace(new RegExp(`${keyframeNames.join("|")}`, "gmi"), (keyframeName) => {
        return obfuscateAttributeName(`bascik__${componentName}__keyframe__${keyframeName}`);
    });
};
 ...
 */
export const convertCssIdSelectorsToClasses = (css, componentName) => {
    const seen = new Map();
    const idsConverted = [];
    const cssStr = css.replace(/#([a-zA-Z][a-zA-Z0-9-_]*)(?=[^{};]*\{)/g, (_, idName) => {
        if (!seen.has(idName)) {
            const className = obfuscateAttributeName(`bascik__${componentName}__id__${idName}`);
 ...
 */
export const addIdClassesInHtml = (html, idsConverted) => {
    if (idsConverted.length === 0)
        return html;
    idsConverted.forEach(({ idName, className }) => {
        const escaped = idName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
        html = html.replace(new RegExp(`(<[^>]+(?<=\\s)id="(?:[^"]*__)?${escaped}"[^>]*)>`, "gi"), (_, tagContent) => {
            if (/\bclass="/.test(tagContent)) {
                return (tagContent.replace(/\bclass="([^"]*)"/, (_, c) => `class="${c} ${className}"`) + ">");
            }
...
export const getComponentCss = async (htmlFileName, cssFileNames) => {
    if (!htmlFileName || !Array.isArray(cssFileNames))
        return;
    const cssFileName = cssFileNames.find((cssFileName) => cssFileName.replace(/\.css$/, ".html") === htmlFileName);
    if (!cssFileName)
        return;
    try {
 ...
 */
export const scopeCssCustomProperties = (css, componentName) => {
    ...
    while ((m = declRegex.exec(css)) !== null) {
        ...
        if (!propMap.has(originalName)) {
            propMap.set(originalName, obfuscateAttributeName(`bascik__${componentName}__${originalName}`));
        }
    }
    if (propMap.size === 0)
        return css;
    ...
    propMap.forEach((scopedName, originalName) => {
        ...
        result = result.replace(new RegExp(`(?<!-)--${originalName}(?=\\s*:)`, "gm"), `--${scopedName}`);
        // Replace var() references:  var(--original)
        result = result.replace(new RegExp(`var\\(\\s*--${originalName}\\s*\\)`, "gm"), `var(--${scopedName})`);
    });
    return result;
};
// ─── @layer Name Scoping ─────────────────────────────────────────────────────
 ...
 */
export const scopeLayerNames = (css, componentName) => {
    const layerNames = new Set();
    css.replace(/@layer\s+([\w-]+(?:\s*,\s*[\w-]+)*)/g, (_, nameList) => {
    ...
    layerNames.forEach((name) => {
        const scoped = obfuscateAttributeName(`bascik__${componentName}__layer__${name}`);
        result = result.replace(new RegExp(`(?<=@layer[^{;]*)\\b${name}\\b`, "gm"), scoped);
    });
    return result;
};
 ...
 */
export const scopeContainerNames = (css, componentName) => {
    const containerNames = new Set();
    css.replace(/container(?:-name)?\s*:\s*([\w-]+)/g, (_, name) => {
        if (name !== "none")
    ...
    containerNames.forEach((name) => {
        const scoped = obfuscateAttributeName(`bascik__${componentName}__container__${name}`);
        result = result.replace(new RegExp(`(?<=@container\\s+)${name}(?=\\s*[({])`, "gm"), scoped);
        result = result.replace(new RegExp(`(container(?:-name)?\\s*:\\s*)${name}`, "gm"), `$1${scoped}`);
    });
    return result;
};
 ...
 */
export const scopeInlineStyleTags = (html, componentName) => {
    const allElementClasses = [];
    const allIdsConverted = [];
    const processedHtml = html.replace(/(<style\b[^>]*>)([\s\S]*?)(<\/style>)/gi, (_match, open, styleContent, close) => {
        let css = removeCommentsFromCss(styleContent);
        css = css.replace(/(?<=\.)[a-z_][a-z0-9-_]*/gim, (className) => obfuscateAttributeName(`bascik__${componentName}__${className}`));
        const { css: elCss, elementsConvertedClasses } = convertCssElementSelectorsToClasses(css, componentName);
        css = elCss;
        allElementClasses.push(...elementsConvertedClasses);
        const { css: idCss, idsConverted } = convertCssIdSelectorsToClasses(css, componentName);
        css = idCss;
 ...
 */
export const deduplicateCss = (usedComponents, dedup = true) => {
    if (!dedup) {
