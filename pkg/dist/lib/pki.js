...
export const createSelfSignedCert = async () => {
    ...
    try {
        ...
        "`);
            await exec(`openssl pkcs12 -in "${pfxPath}" -nodes -nocerts -out "${keyPath}" -passin pass:`);
            await exec(`openssl pkcs12 -in "${pfxPath}" -nokeys -out "${certPath}" -passin pass:`);
            await rm(pfxPath, { force: true });
        }
        else {
            await exec(`openssl req -x509 -newkey rsa:2048 -nodes -sha256 -days 36500 \
        -subj "/CN=localhost" \
