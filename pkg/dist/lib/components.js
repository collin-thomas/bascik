...
export const listComponents = async () => {
    const componentFileNames = (await deepReadDirFlat(BascikConfig.directory.components, /\.(html|css)$/)) ?? [];
    const componentHtmlFileNames = componentFileNames.filter((fileName) => fileName.match(/\.(html)$/));
    const componentCssFileNames = componentFileNames.filter((fileName) => fileName.match(/\.(css)$/));
    const components = await Promise.all(componentHtmlFileNames.map(async (fileName) => {
        ...
        try {
            ...
            if (NATIVE_HTML_ELEMENTS.has(componentName)) {
                console.warn(`warning: Component "${componentName}" has the same name as a native HTML element. ` +
            ...
            console.warn(`warning: Failed to process ${fileName}`, ...(BascikConfig.verboseLogging ? [{ cause: e }] : []));
            return {};
        }
    }));
...
export const replaceTag = (htmlString, tagName, transpiledTag) => {
    ...
    const pairedRegexp = new RegExp(`(?<content><${tagName}[^>]*>(?<innerContent>([\\s\\S]*?))<\/${tagName}>)`, "i");
...
export const getTagContents = (htmlString, tagName) => {
    const regexp = new RegExp(`(?<content><${tagName}[^>]*>(?<innerContent>([\\s\\S]*?))<\/${tagName}>)`, "i");
...
export const getFirstComponent = (htmlString, componentList) => {
    ...
    const componentNames = Object.keys(componentList).sort((a, b) => b.length - a.length);
    const matchComponentName = new RegExp(`<\\b(${componentNames.join("|")})\\b[\\s\\S]*?>`, "i");
...
export const getTag = (htmlString, tagName, componentList) => {
    ...
    const pairedPattern = new RegExp(`<${tagName}[\\s\\S]*?>([\\s\\S]*?)<\\/${tagName}>`, "i");
...
export const minifyHtml = (htmlString) => {
    ...
    const preserved = [];
    html = html.replace(/<(pre|textarea)([ \t][^>]*)?>[\s\S]*?<\/\1>/gi, (match) => {
    ...
    if (preserved.length) {
        ...
        html = html.replace(/\x00P(\d+)\x00/g, (_, i) => preserved[parseInt(i, 10)]);
    }
    if (scriptTags) {
        html += `\n${scriptTags}`;
    }
 ...
 */
export const extractProps = (componentContent) => {
    const props = {};
    if (!componentContent)
        return props;
 ...
 *      → `<p>Hi</p>`
 */
export const injectProps = (fileContent, props) => {
    if (!fileContent)
        return "";
    ...
    Object.entries(props).forEach(([propName, propValue]) => {
        ...
        result = result.replace(new RegExp(`<(\\w+(?:-\\w+)*)([^>]*?)\\s+${attrName}([^>]*)>(.*?)<\\/\\1>`, "gi"), (_match, tagName, attrsBefore, attrsAfter, _oldContent) => `<${tagName}${attrsBefore}${attrsAfter}>${propValue}</${tagName}>`);
    });
 ...
 */
const findMatchingClose = (html, tagName, contentStart) => {
 ...
 */
const parseNamedSlots = (innerContent) => {
    const results = [];
 ...
 *      → '<p>body</p>'
 */
export const extractDefaultSlotContent = (innerContent) => {
    if (!innerContent)
        return "";
 ...
 *      → { header: "<h1>H</h1>" }
 */
export const extractNamedSlotContent = (innerContent) => {
    if (!innerContent)
        return {};
    const slots = {};
 ...
 */
export const replaceNamedSlots = (fileContent, slots) => {
    return fileContent.replace(new RegExp(`<(\\w+(?:-\\w+)*)\\s+data-bascik-slot="([^"]+)"[^>]*>([\\s\\S]*?)<\\/\\1>`, "gi"), 
    // Use the element's own inner content as fallback when the slot is not provided
    (_match, _tag, slotName, innerFallback) => slotName in slots ? slots[slotName] : innerFallback);
};
// ─────────────────────────────────────────────────────────────────────────────
 ...
 */
export const extractInheritableAttributes = (componentContent) => {
    if (!componentContent)
        return {};
    const attrs = {};
 ...
 */
export const mergeAttributesOntoRoot = (html, attrs) => {
    if (!Object.keys(attrs).length)
        return html;
    return html.replace(/^(<[\w-]+)((?:\s[^>]*?)?)(\s*\/?>)/, (_match, tagName, existing, close) => {
        let attrStr = existing || "";
        for (const [name, value] of Object.entries(attrs)) {
            if (name === "class" && value) {
                if (attrStr.includes('class="')) {
                    attrStr = attrStr.replace(/class="([^"]*)"/, (_, cls) => `class="${cls} ${value}"`);
                }
                else {
                    attrStr += ` class="${value}"`;
                }
